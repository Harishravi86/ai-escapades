...
"""
================================================================================
BULLETPROOF STRATEGY v7.3 - ASTRO-ML INTEGRATED
================================================================================

Based on NEAT Evolution findings: Let ML learn from CONTINUOUS celestial 
separations rather than binary flags or hardcoded boosts.

Changes from v7.2:
- REMOVES hardcoded `moon_opp_uranus` sizing boost
- REMOVES hardcoded `is_lunar_window` sizing boost  
- ADDS sun_saturn_sep_normalized (0-1) as ML feature
- ADDS moon_uranus_sep_normalized (0-1) as ML feature
- ADDS spread_normalized (planetary spread) as ML feature
- ADDS Spread Regime position sizing (Danger Zone = -20%)
- KEEPS validated exit override rule (+1.5% alpha)

Key insight from NEAT:
- Continuous separations > Binary flags
- sun_saturn_sep weight: +1.84 (hold signal)
- moon_uranus_sep weight: -0.83 (entry signal near opposition)

The ML model will now learn these weights automatically.
================================================================================
"""
...
class CelestialEngine:
    """
    v7.3: Complete celestial calculation engine.
    ...
    def _calculate_features(self, date_str: str) -> Dict:
        """Internal calculation of all features."""
        # Parse date
...
class TechnicalEngine:
    """
    v7.3: Adds NEAT-discovered celestial features to ML inputs.
...
# =============================================================================
# BULLETPROOF STRATEGY v7.3 - ASTRO-ML
# =============================================================================

class BulletproofStrategyV73:
    """
    v7.3 Astro-ML:
    
    Key Changes from v7.2:
    1. Celestial features are ML inputs (not hardcoded boosts)
    2. Spread regime affects position sizing (Danger Zone = -20%)
    3. Exit override rule KEPT (validated +1.5% alpha)
    
    Removed:
    - moon_opp_uranus sizing boost → ML learns this
    - is_lunar_window sizing boost → ML learns this
    """
    ...
    def backtest(self, params: Optional[Dict] = None, initial_capital: float = 100000, verbose: bool = True):
        """
        v7.3 backtest with:
        - ML-learned celestial features
        - Spread regime position sizing
        - Validated exit override
        """
        if params is None:
            params = {
                'bull_threshold': 0.45,
        ...
        for i, date in enumerate(self.signal_data.index):
            ...
            if bull_signal or sharktooth_signal:
                ...
                # =============================================================
                # v7.3: SPREAD REGIME POSITION SIZING (replaces hardcoded boosts)
                # =============================================================
                if signal_size > 0:
    ...
    def print_results(self):
        s = self.stats
        print("\n" + "=" * 70)
        print("BULLETPROOF STRATEGY v7.3 - ASTRO-ML")
        print("(Celestial features learned by ML + Spread Regime Sizing)")
        print("=" * 70)
    ...
    def print_feature_importance(self):
        ...
        print("\n" + "=" * 70)
        print("FEATURE IMPORTANCE ANALYSIS (v7.3)")
        print("=" * 70)
    ...
    def save_models(self, directory: str = '.'):
        ...
        if self.bull_detector:
            self.bull_detector.save(os.path.join(directory, 'bull_v73_astro.joblib'))
        if self.bear_detector:
            self.bear_detector.save(os.path.join(directory, 'bear_v73_astro.joblib'))
...
if __name__ == "__main__":
    ...
    print("=" * 70)
    print("\nKey changes from v7.2:")
    print("  ✓ Celestial separations as ML features (not hardcoded)")
    print("  ✓ Spread regime position sizing (Danger Zone = -20%)")
    print("  ✓ Exit override rule preserved (+1.5% alpha)")
    print("  ✗ Removed hardcoded moon_opp_uranus boost")
    print("  ✗ Removed hardcoded is_lunar_window boost")
    print("=" * 70)
    ...
    try:
        ...
        strategy_spy = BulletproofStrategyV73(spy, spy, vix, symbol='SPY')
        ...
        strategy_qqq = BulletproofStrategyV73(spy, qqq, vix, symbol='QQQ')
        ...
        if strategy_spy.trades:
            ...
            trades_df.to_csv('v7_3_trades_spy.csv', index=False)
            print("\nSaved trades to v7_3_trades_spy.csv")
