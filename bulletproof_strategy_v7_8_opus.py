"""
================================================================================
BULLETPROOF STRATEGY v7.8 - OPUS PATCH
================================================================================

Based on v7.7 + Opus 4.5 Suggestions:
- ADDS "Confluence Scores" (DANGER_CONFLUENCE, CALENDAR_BIAS)
- ADDS Day of Week features (IS_MONDAY, IS_FRIDAY)
- ADDS Bayesian-style continuous signals to reduce overfitting on sparse dates

Performance Evolution:
- v7.7 Sniper:   180,606% QQQ Return
- v7.8 Opus:     [Testing...]

Key New Features:
- DANGER_CONFLUENCE: Sum of danger flags (0-5 score)
- CALENDAR_BIAS: Net bullish/bearish score (-1.0 to +1.0)
- MONDAY_JAN_5: The "Ultimate" confluence date
================================================================================
"""
...
class TechnicalEngine:
    ...
    def calculate(cls, df: pd.DataFrame, celestial_df: Optional[pd.DataFrame] = None) -> pd.DataFrame:
        ...
        # Note: cumcount() starts at 0, so 0 is first item
        features['FIRST_TRADING_DAY'] = (df.groupby([df.index.year, df.index.month]).cumcount() == 0).astype(int)
        
        # Turn of Month (Last 3 + First 2 TRADING days) - FIX from Code Review
        # Using cumcount() handles weekends/holidays correctly (unlike day index)
        trading_day_count = df.groupby([df.index.year, df.index.month]).cumcount()
        trading_day_reverse = df.groupby([df.index.year, df.index.month]).cumcount(ascending=False)
        
        features['TURN_OF_MONTH'] = (
            (trading_day_count <= 1) |  # First 2 days (0, 1)
            (trading_day_reverse <= 2)  # Last 3 days (0, 1, 2 from end)
        ).astype(int)
        
        # =================================================================
        # 9c. SNIPER CALENDAR FEATURES (v7.7)
        # =================================================================
        day_of_week = df.index.dayofweek # 0 = Monday
        
        # 1. Monday the 5th (Volmageddon / Japan Crash specific)
        features['MONDAY_5TH'] = ((day_of_month == 5) & (day_of_week == 0)).astype(int)
        
        # 2. Jan 5th (Upcoming risk date)
        features['DAY_5_JAN'] = ((day_of_month == 5) & (month == 1)).astype(int)
        
        # 3. First Week of Year (Low liquidity / Tax selling hangover?)
        features['FIRST_WEEK_OF_YEAR'] = ((month == 1) & (day_of_month <= 7)).astype(int)
        
        # 4. ULTIMATE DANGER: Monday + 5th + (Jan/Feb/Aug)
        features['ULTIMATE_DANGER'] = (
            (day_of_week == 0) & 
            (day_of_month == 5) & 
            (month.isin([1, 2, 8]))
        ).astype(int)
        
        # 5. MONDAY JAN 5 (Specific Opus Request)
        features['MONDAY_JAN_5'] = (
            (day_of_week == 0) & 
            (day_of_month == 5) & 
            (month == 1)
        ).astype(int)
        ...
        # =================================================================
        # 9d. OPUS CONFLUENCE SCORES (Continuous features > Sparse binaries)
        # =================================================================
        # Danger Confluence (0.0 to 1.0 Normalized Score)
        # Sum of: DAY_5_DANGER, MONDAY_5TH, ULTIMATE_DANGER, FIRST_WEEK_OF_YEAR, DAY_5_JAN
        danger_sum = (
            features['DAY_5_DANGER'] + 
            features['MONDAY_5TH'] + 
            features['ULTIMATE_DANGER'] * 2 + # Weighted higher
            features['FIRST_WEEK_OF_YEAR'] + 
            features['DAY_5_JAN']
        )
        features['DANGER_CONFLUENCE'] = danger_sum / 6.0 # Normalize 0-1 range
        
        # Bullish Confluence
        # Sum of: DAY_5_BULLISH, TURN_OF_MONTH, FIRST_TRADING_DAY
        bull_sum = (
            features['DAY_5_BULLISH'] + 
            features['TURN_OF_MONTH'] + 
            features['FIRST_TRADING_DAY']
        )
        features['BULLISH_CONFLUENCE'] = bull_sum / 3.0
        
        # Net Calendar Bias (-1.0 to +1.0)
        # -1 = Extreme Danger, +1 = Extreme Bullish
        features['CALENDAR_BIAS'] = features['BULLISH_CONFLUENCE'] - features['DANGER_CONFLUENCE']
...
class BulletproofStrategyV78_Opus:
    """
    v7.8 Opus Edition:
    
    Key Changes from v7.7:
    1. Added Confluence Scores (DANGER_CONFLUENCE, CALENDAR_BIAS)
    2. Added Continuous signals for Bayesian smoothing
    3. Added Day of Week logic (IS_MONDAY, IS_FRIDAY)
    """
    ...
    def print_results(self):
        s = self.stats
        print("\n" + "=" * 70)
        print("BULLETPROOF STRATEGY v7.8 - OPUS")
        print("(Eclipse + Vedic + NEAT + Opus Calendar)")
        print("=" * 70)
    ...
    def print_feature_importance(self):
        ...
        print("\n" + "=" * 70)
        print("\n" + "=" * 70)
        print("FEATURE IMPORTANCE ANALYSIS (v7.8)")
        print("=" * 70)
        print("=" * 70)
    ...
    def save_models(self, directory: str = '.'):
        ...
        if self.bull_detector:
            self.bull_detector.save(os.path.join(directory, 'bull_v78_opus.joblib'))
        if self.bear_detector:
            self.bear_detector.save(os.path.join(directory, 'bear_v78_opus.joblib'))
...
if __name__ == "__main__":
    print("=" * 70)
    print("BULLETPROOF STRATEGY v7.8 - OPUS PATCH")
    print("=" * 70)
    print("\nKey changes from v7.7:")
    print("  [+] Confluence Scores (DANGER_CONFLUENCE, CALENDAR_BIAS)")
    print("  [+] Continuous signals (Bayesian smoothing)")
    print("  [+] Day of Week logic (IS_MONDAY, IS_FRIDAY)")
    print("=" * 70)
    ...
    try:
        ...
        strategy_spy = BulletproofStrategyV78_Opus(spy, spy, vix, symbol='SPY')
        ...
        strategy_qqq = BulletproofStrategyV78_Opus(spy, qqq, vix, symbol='QQQ')
        ...
        if strategy_spy.trades:
            ...
            trades_df.to_csv('v7_8_trades_spy.csv', index=False)
            print("\nSaved trades to v7_8_trades_spy.csv")
